...
3. [Supporting Services](./03-Supporting-Services/README.md): this module is used to deploy the supporting services that the ARO cluster will need.
    ...
    - Disk Encryption Set (Optional): used to encrypt the disks of the virtual machines in the ARO cluster with Customer Managed Key (CMK). The DES is not deployed by default. By default the Disk Encryption Set (DES) is not deployed. For workers, you can bring your own key (BYOK) after the deployment of the cluster. If you need CMK for the master nodes, you need to deploy a DES when creating the cluster. See: [Encrypt OS disks with a customer-managed key on Azure Red Hat OpenShift](https://learn.microsoft.com/en-us/azure/openshift/howto-byok#create-an-azure-red-hat-openshift-cluster)
